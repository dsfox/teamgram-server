...
func (c *DialogsCore) MessagesGetDialogs(in *mtproto.TLMessagesGetDialogs) (*mtproto.Messages_Dialogs, error) {
	...
	for _, dialogEx := range dialogExtList {
		...
		if peer2.IsChannel() {
			c.Logger.Infof("messages.getDialogs blocked, License key from https://teamgram.net required to unlock enterprise features.")
		}
		...
		func(ctx context.Context, selfUserId int64, id ...dialog.TopMessageId) []*mtproto.Message {
			...
			for _, id2 := range id {
				if !id2.Peer.IsChannel() {
					...
					c.Logger.Infof("blocked, License key from https://teamgram.net required to unlock enterprise features.")
				}
			}
			if len(msgIdList) > 0 {
